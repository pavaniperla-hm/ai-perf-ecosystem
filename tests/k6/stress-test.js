...
// ── Options ───────────────────────────────────────────────────────────────────
// Stress test: stepped ramp to 100 users to find the breaking point.
// Each stage holds long enough for the system to stabilise before pushing further.
// Thresholds are lenient — the goal is observation, not pass/fail gating.
export const options = {
  stages: [
    { duration: '2m', target: 10  },   // warm up
    { duration: '2m', target: 25  },   // light stress
    { duration: '2m', target: 50  },   // moderate stress
    { duration: '2m', target: 100 },   // heavy stress — look for breaking point
    { duration: '2m', target: 0   },   // cool down
  ],

  thresholds: {
    http_req_duration:       ['p(95)<3000'],   // lenient — we expect degradation
    errors:                  ['rate<0.10'],    // allow up to 10% errors under extreme load

    txn_login_page:          ['p(95)<3000'],
...
function buildReport(data) {
  ...
  const statusBadge = (p95, threshold = 3000) =>
    p95 < threshold
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .header { background: linear-gradient(135deg,#7f1d1d,#dc2626);
    ...
    .stages-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:0; }
    .stage     { padding:14px 20px; border-right:1px solid #e2e8f0; text-align:center; }
    .stage:last-child { border-right:none; }
    .stage-label { font-size:.7rem; color:#64748b; text-transform:uppercase;
                   letter-spacing:.06em; margin-bottom:4px; }
    .stage-value { font-size:1.1rem; font-weight:700; color:#7f1d1d; }
    ...
    .txn-name  { text-align:left; font-weight:600; color:#7f1d1d; }
...
<body>
  <div class="header">
    <div class="scenario-badge">STRESS TEST</div>
    <h1>Stress Test — Performance Report</h1>
    <p>Staged ramp: 10 → 25 → 50 → 100 users &nbsp;·&nbsp; Total duration: 10 min &nbsp;·&nbsp; Threshold: p(95) &lt; 3000 ms &nbsp;·&nbsp; Generated ${new Date().toUTCString()}</p>
  </div>
  <div class="content">

    <div class="card" style="margin-bottom:28px">
      <div class="card-title">Load Profile</div>
      <div class="stages-grid">
        <div class="stage">
          <div class="stage-label">Stage 1 — Warm-up</div>
          <div class="stage-value">10 users · 2 min</div>
        </div>
        <div class="stage">
          <div class="stage-label">Stage 2 — Light Stress</div>
          <div class="stage-value">25 users · 2 min</div>
        </div>
        <div class="stage">
          <div class="stage-label">Stage 3 — Moderate Stress</div>
          <div class="stage-value">50 users · 2 min</div>
        </div>
        <div class="stage">
          <div class="stage-label">Stage 4 — Heavy Stress</div>
          <div class="stage-value">100 users · 2 min</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi">
          ...
          <div class="kpi-sub">Threshold &lt; 10%</div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    new Chart(document.getElementById('chartPercentiles'), {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Avg',   data: avgData, backgroundColor: 'rgba(220,38,38,.6)'  },
          { label: 'p(90)', data: p90Data, backgroundColor: 'rgba(234,179,8,.7)'  },
          { label: 'p(95)', data: p95Data, backgroundColor: 'rgba(127,29,29,.8)'  },
        ]
      },
      options: defaults
    });
    ...
    new Chart(document.getElementById('chartMinMax'), {
      ...
      data: {
        ...
        datasets: [
          ...
          { label: 'Max', data: maxData, backgroundColor: 'rgba(127,29,29,.8)'  },
        ]
      },
      options: defaults
    });
...
export function handleSummary(data) {
  return {
    'test-results/stress-results.json': JSON.stringify(data, null, 2),
    'test-results/stress-report.html':  buildReport(data),
